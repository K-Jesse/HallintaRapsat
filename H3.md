...
Tehdään aluksi jokin virhe. `$ micro tyhma_tiedosto.md` Lisätään se vielä. `$ git add .` Voi ei! Käytetään resettiä korjaamaan virhe. `$ git reset --hard` Katsotaan vielä, että tilanne on korjattu. `$ git status`
